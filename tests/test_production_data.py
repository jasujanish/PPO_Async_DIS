...
def test_prepared_curriculum_has_400_unique_problems_and_one_pass_budget():
    ...
    assert len(rows) == len({r["metadata"]["statement_hash"] for r in rows}) == 400
    assert Counter(r["metadata"]["source_name"] for r in rows) == {
        "lean-workbook": 200, "proofnet-verified": 200,
    }
    assert report["training_examples"] == 400
    assert report["selection"]["passes_per_dataset"] == 1
    assert report["processed_example_budget"] == 400
...
def test_materialize_final_selects_first_200_per_source_before_shuffle(tmp_path, monkeypatch):
    ...
    assert {r["metadata"]["problem_id"] for r in rows} == {
        f"{source}-{i}" for source in sources for i in range(200)
    }
    assert report["processed_example_budget"] == 400
